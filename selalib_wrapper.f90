...
contains
  ...
  subroutine interpolate_2d(N1, N2, data_in, &
    ...
    REAL(KIND=8), intent(in), dimension(:, :) :: char_x1
    REAL(KIND=8), intent(in), dimension(:, :) :: char_x2
    REAL(KIND=8), intent(in), dimension(:, :) :: data_in
    REAL(KIND=8) :: zero_var = 0.0
    REAL(KIND=8) :: one_var = 1.0
    REAL(KIND=8), dimension(2,N2) :: eta1_slopes
    REAL(KIND=8), dimension(2,N1) :: eta2_slopes
    REAL(KIND=8), intent(out), dimension(N1,N2) :: data_exit
    class(sll_c_interpolator_2d), pointer :: cs2d
    type(sll_t_cubic_spline_interpolator_2d), target    :: spline

    !-------------------------------------------------------
    !f2py INTEGER(KIND=4), intent(in,hide)  :: N1, N2
    !f2py REAL(KIND=8), intent(in), dimension(N1, N2) :: data_in
    !--------------------------------------------------------

    call spline%init(N1,N2, &
         zero_var, one_var, zero_var, one_var,&
  ...
  subroutine alt_interpolate_2d(N1, N2, data_in, &
    ...
    REAL(KIND=8), dimension(2,N2) :: eta1_slopes
    REAL(KIND=8), dimension(2,N1) :: eta2_slopes
    REAL(KIND=8), intent(out), dimension(N1,N2) :: data_out
    class(sll_c_interpolator_2d), pointer :: cs2d
    type(sll_t_cubic_spline_interpolator_2d), target    :: spline
    INTEGER(KIND=4) :: i, j
    REAL(KIND=8)    :: eta1
    REAL(KIND=8)    :: eta2
    REAL(KIND=8), dimension((N1+2)*(N2+2)) :: coeffs
    ...
    coeffs(:) = 0.d+0
    call sll_s_cubic_spline_2d_get_coeff(spline%spline, coeffs)
  ...
  subroutine interpolate_value(NPTS1, NPTS2, &
    ...
    INTEGER(KIND=4), intent(in) :: NPTS1, NPTS2
    REAL(KIND=8), dimension(2,NPTS1), optional :: eta1_slopes
    REAL(KIND=8), dimension(2,NPTS2), optional :: eta2_slopes
    REAL(KIND=8), intent(in), dimension(NPTS1,NPTS2) :: data
    ...
    !-------------------------------------------------------
    !f2py INTEGER(KIND=4), intent(in,hide)  :: NPTS1, NPTS2
    !f2py REAL(KIND=8), intent(in), dimension(2, NPTS2), optional :: eta1_slopes
    !f2py REAL(KIND=8), intent(in), dimension(2, NPTS1), optional :: eta2_slopes
    !--------------------------------------------------------
  ...
  subroutine poisson_solver_periodic(N1, N2, rho, phi)
    ...
    REAL(KIND=8),    intent(in),  dimension(:, :)   :: rho
    REAL(KIND=8),    intent(out), dimension(N1, N2) :: phi
